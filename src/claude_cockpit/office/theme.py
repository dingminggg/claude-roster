...
# ---------- 桌上那部手机(= 会话间对话记录的入口) ----------
# **不上饱和色**:饱和色只留给显示器和小人;手机就是桌上一块深灰的小板子,
# 亮的是屏幕面。红点是唯一的例外——它是「有没看过的消息」,必须一眼看见,
# 但它**不闪**(半拍一闪那拍归显示器,两样都闪就分不清谁在叫你)。
PHONE_BODY = QColor("#3f4753")      # 机身(顶面边框)
PHONE_SCREEN = QColor("#8f9aa8")    # 屏幕面:比机身浅一档,才看得出是块屏
PHONE_SIDE = QColor("#333a44")      # 两个立面:比顶面深,盒子才立得起来
PHONE_DOT = QColor("#ef4444")       # 未读红点
